...
describe("shouldUseMockClient", () => {
  ...
  it("detects Firefox browser native messaging as an installed extension context", () => {
    (globalThis as { browser?: unknown }).browser = {
      runtime: {
        sendNativeMessage: async () => ({ id: "version-1", result: { version: "0.1.9", platform: "win32" } })
      }
    };
    ...
    (globalThis as { browser?: unknown }).browser = {
      runtime: {
        sendNativeMessage: async (_hostName: string, message: { id: string; method: string }) => ({
          id: message.id,
          result: { version: "0.1.9", platform: "win32" }
        })
      }
    };

    await expect(createNativeHostClient().version()).resolves.toEqual({ version: "0.1.9", platform: "win32" });
  });
