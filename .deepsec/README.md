...
Currently configured project: `LocalhostControl` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/LocalhostControl/SETUP.md` to fill in
   `data/LocalhostControl/INFO.md`.
...
`--project-id LocalhostControl` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/LocalhostControl/`.
