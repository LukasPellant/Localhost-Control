...
if (targets.length === 0) {
  throw new Error(`Unsupported native host packaging platform: ${process.platform}`);
}
...
try {
  ...
  run(process.execPath, ["scripts/validate-native-host-current.mjs"]);
  run(process.execPath, ["scripts/write-native-host-checksums.mjs"]);
} catch (error) {
